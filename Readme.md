...
На первом этапе мы обучаем базовую модель, ориентируясь на максимальный **recall** (полноту).Это связано с поставленной задачей:
...
В исходных данных `RuReady.csv` присутствует шум: часть дагестанцев размечены как «0» (русские).Это негативно влияет на:
...
- Финальные модели, обученные на очищенных данных datafortrainUp4 (модель: [BalancedData_50000.pkl](main_folder/ModelsUP/modelsUp4/LogReg095/(2-3)_iter500_C0.15/model/BalancedData_50000.pkl)).
